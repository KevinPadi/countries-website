...
function SearchBar ({ searchCountry, setSearchCountry }) {
  return (
    <div className='w-full sm:w-[330px] lg:w-[450px] relative'>
      <input value={searchCountry} onChange={(e) => setSearchCountry(e.target.value)} type='text' name='country' className='bg-white dark:bg-darkBlue w-full h-10 px-20 py-7 block placeholder:text-sm placeholder:text-darkGray placeholder:dark:text-white placeholder:font-semibold font-semibold rounded-md drop-shadow-md' placeholder='Search for a country...' />
      <div
        className='absolute inset-y-0 left-5 flex items-center pl-3 pointer'
      >
